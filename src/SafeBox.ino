...
if (key == '*') {
    beep(100);
    Serial.println("[ACCESS] '*' pressed. Exiting AP mode.");
    toggleApMode();
    currentState = GRANT_ACCESS_AFTER_AP;
...
void loadPassword() {
    ...
    if (!hasValidData || password.length() == 0) {
        Serial.println("[EEPROM] No valid password found. Using default and saving.");
